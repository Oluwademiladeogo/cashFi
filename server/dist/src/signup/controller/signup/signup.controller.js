...
let SignupController = class SignupController {
    ...
    async SignupUser(user, res) {
        const userObject = await userschema_1.userModel.findOne({ email: user.email });
        if (userObject)
        ...
        try {
            ...
            throw new common_1.HttpException("Error saving user details", common_1.HttpStatus.REQUEST_TIMEOUT);
        }
